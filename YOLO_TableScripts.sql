...
-- 회원 추가
-- INSERT INTO YOLO_MEMBER (NO, ID, PASSWORD, NAME, PCODE, ADDRESS1, ADDRESS2, PHONE, EMAIL, STATUS, CREATE_DATE) VALUES (YOLO_MEMBER_SEQ.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, 'Y', SYSDATE);

-- 회원 수정
-- UPDATE YOLO_MEMBER SET NAME=?, PCODE=?, ADDRESS1=?, ADDRESS2=?, PHONE=?, EMAIL=?, UPDATE_DATE=SYSDATE WHERE ID=?;
...
CREATE TABLE YOLO_PROGRAM (
    ...
    WRITER_NAME VARCHAR2(16),                -- 작성자                                        
    TITLE VARCHAR2(40),                      -- 모임명
    THUMB VARCHAR2(500) NOT NULL,            -- 썸네일(대표이미지)
...
COMMENT ON COLUMN YOLO_BOARD.THUMB IS '썸네일';
